...
- [x] 1.1 Inventory responsibilities in `src/main.rs` and map them to target modules
...
- [x] 2.3 Extract HTTP client/preview handlers into a `http_client` module (public API unchanged)
- [x] 2.4 Extract code lens/action/hover handlers into a `lsp_handlers` module
- [x] 2.5 Extract semantic tokens and symbol/folding helpers into a `analysis` module

## 3. Context And Wiring

- [x] 3.1 Introduce a context struct that owns shared maps and LSP client
- [x] 3.2 Update handlers to use context accessors instead of passing state around
...
- [x] 5.1 Sanity-check behavior parity for commands, code lens/action, hover, and diagnostics
