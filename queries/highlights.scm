; Vendored from tree-sitter-idl 3.18.0 (queries/highlights.scm), used by the
; language server's semantic-token highlighting instead of the crate-provided
; HIGHLIGHTS_QUERY constant, so upstream query regressions cannot silently break
; highlighting here.
;
; The one deliberate difference: upstream's comment rule is `(comment) @comment @spell`.
; tree-sitter-highlight makes the LAST capture on a node win, so `@spell` (which is
; not a configured semantic-token capture here) would win and comments would end up
; with no highlight at all. This fork drops @spell entirely: the language server
; only emits semantic tokens and has no spell-check consumer.
